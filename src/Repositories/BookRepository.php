...
final class BookRepository
{
    ...
    public function all(string $q = '', int $limit = 0): array
    {
        ...
        $args = [];
        if ($q !== '') {
            // EMULATE_PREPARES = false → each named placeholder must be unique.
    ...
    public function create(array $b, int $createdBy): int
    {
        ...
        $stmt->execute([
            ...
            ':year'   => (int)$b['year'],
            ':genre'  => trim((string)($b['genre'] ?? 'Uncategorised')),
            ':owner'  => $createdBy,
        ]);
    ...
    public function update(int $id, array $b): int
    {
        ...
        if (!$sets) return 0;
        $stmt = $this->pdo->prepare('UPDATE books SET ' . implode(',', $sets) . ' WHERE id=:id');
        $stmt->execute($args);
